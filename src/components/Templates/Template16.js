...
export const Template16 = ({ formData }) => {
  const pdf = new jsPDF({
  ...
  try {
    ...
    // Contact info line - smaller, centered
    pdf.setFont("helvetica", "normal");
    pdf.setFontSize(8);
    pdf.setTextColor(100, 100, 100);

    let contactParts = [];
    if (personalDetails.phoneNumber) contactParts.push(`Contact No : ${personalDetails.phoneNumber}`);

    if (contactParts.length > 0) {
        const contactLine1 = contactParts.join(" • ");
        const contactWidth1 = pdf.getTextWidth(contactLine1);
        pdf.text(contactLine1, (pageWidth - contactWidth1) / 2, yPosition);
        yPosition += 4;
    }

    let contactParts2 = [];
    if (personalDetails.email) contactParts2.push(`Email : ${personalDetails.email}`);
    if (personalDetails.linkedin) contactParts2.push(`Linkedin: ${personalDetails.linkedin}`);
    
    if (contactParts2.length > 0) {
      const contactLine2 = contactParts2.join(" • ");
      const contactWidth2 = pdf.getTextWidth(contactLine2);
      pdf.text(contactLine2, (pageWidth - contactWidth2) / 2, yPosition);
      yPosition += 4;
    }
    ...
    const addSectionHeader = (title, number) => {
      ...
      pdf.circle(leftMargin + 4, yPosition, 4, 'F');
      
      // White number in circle
      pdf.setTextColor(255, 255, 255);
      pdf.setFont("helvetica", "bold");
      pdf.setFontSize(12);
      const numWidth = pdf.getTextWidth(number.toString());
      pdf.text(number.toString(), leftMargin + 4 - (numWidth/2), yPosition + 1);
      
      // Section title
      pdf.setTextColor(139, 69, 19);
      pdf.setFont("helvetica", "bold");
      pdf.setFontSize(14);
      pdf.text(title, leftMargin + 15, yPosition);
    ...
    if (checkStr(personalDetails.about)) {
      addSectionHeader("Profile", 1);
      
      pdf.setTextColor(0, 0, 0);
      pdf.setFont("helvetica", "normal");
      pdf.setFontSize(10);
      
      const profileText = pdf.splitTextToSize(personalDetails.about, contentWidth - 20);
      pdf.text(profileText, leftMargin + 5, yPosition);
      yPosition += profileText.length * 4 + 15;
    }

    // 2. EDUCATION SECTION
    if (checkEach(educationDetails, "collegeName") || 
        checkEach(educationDetails, "course") ||
        checkEach(educationDetails, "year")) {
      
      addSectionHeader("Education", 2);
      ...
      educationDetails.forEach((edu, index) => {
        ...
        pdf.setTextColor(100, 100, 100);
        if (edu.year) {
          pdf.text(edu.year, leftMargin + 5, yPosition);
        }
        ...
        pdf.setFontSize(10);
        pdf.setTextColor(0, 0, 0);
        let collegeInfo = edu.collegeName || "";
        if (edu.location) {
    ...
    if (checkEach(experienceDetails, "companyName") ||
      ...
      experienceDetails.forEach((exp, index) => {
        ...
        pdf.setTextColor(100, 100, 100);
        if (exp.year) {
          pdf.text(exp.year, leftMargin + 5, yPosition);
        }
        ...
        pdf.setFontSize(10);
        pdf.setTextColor(0, 0, 0);
        let companyInfo = exp.companyName || "";
        if (exp.location) {
    ...
    if (checkEach(skills, "skillName")) {
      addSectionHeader("Skills", 4);
      
      // Group skills by category or show as single list
      const skillCategories = {};
      
      skills.forEach(skill => {
        const category = skill.category || "LANGUAGES"; // Default category
        if (!skillCategories[category]) {
          skillCategories[category] = [];
        }
        skillCategories[category].push(skill.skillName);
      });
      ...
      Object.keys(skillCategories).forEach(category => {
        ...
        pdf.setFontSize(10);
        pdf.setTextColor(0, 0, 0);
        pdf.text(category.toUpperCase(), leftMargin + 5, yPosition);
        ...
        const skillsList = skillCategories[category].join(" • ");
        const skillsWrapped = pdf.splitTextToSize(skillsList, contentWidth - 15);
    ...
    if (checkEach(certification, "certiName") || checkEach(certification, "year")) {
      ...
      certification.forEach((cert, index) => {
        ...
        pdf.setTextColor(100, 100, 100);
        if (cert.year) {
          pdf.text(cert.year, leftMargin + 5, yPosition);
        }
    ...
    if (checkEach(projectDetails, "projectName")) {
      addSectionHeader("Projects", 6);
      ...
      projectDetails.forEach((project, index) => {
        ...
        pdf.setTextColor(100, 100, 100);
        if (project.year) {
          pdf.text(project.year, leftMargin + 5, yPosition);
        }
